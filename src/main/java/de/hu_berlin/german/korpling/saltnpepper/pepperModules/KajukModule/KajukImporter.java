...
public class KajukImporter extends PepperImporterImpl implements PepperImporter
{
			...
			throws PepperModuleException
	{
		...
		if (this.getSCorpusGraph()== null)
			throw new KajukImporterException(this.name+": Cannot start with importing corpus, because salt project isn�t set.");
		if (this.getCorpusDefinition()== null)
			throw new KajukImporterException(this.name+": Cannot start with importing corpus, because no corpus definition to import is given.");
		if (this.getCorpusDefinition().getCorpusPath()== null)
			throw new KajukImporterException(this.name+": Cannot start with importing corpus, because the path of given corpus definition is null.");
		...
		if (this.getCorpusDefinition().getCorpusPath().isFile())
		{
			...
			try {
				...
				throw new KajukImporterException(this.name+": Cannot start with importing corpus, because saome exception occurs: ",e);
			}
		}	
	}
	...
	public void start(SElementId sElementId) throws PepperModuleException 
	{
		...
		{	
			...
			this.getLogService().log(LogService.LOG_WARNING, arg1);
			...
			if (documentPath!= null)
			{
				SDocument sDoc= (SDocument) sElementId.getSIdentifiableElement();
				{
					...
					mapper.setCorpusPath(new File(this.getCorpusDefinition().getCorpusPath().toFileString()));
					mapper.start(sDoc);
				}	
			}
		}
